...
`git clone https://github.com/ByteLegendQuest/java-implement-type-casts`

A project directory named `java-implement-type-casts` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-implement-type-casts` directory which is created just now.
